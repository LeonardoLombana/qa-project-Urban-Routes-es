...
class UrbanRoutesPage:
    ...
    def set_clic_btn_siguient_telefono(self):
        btnSiguiente = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.btn_siguiente_telefono))
        btnSiguiente.click()

    def set_agregar_codigo(self, codigo):
        agregarCodigo = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.input_ingresar_codigo))
    ...
    def set_clic_modo_pago(self):
        seleccionar_tarjeteCredito = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.clic_modo_de_pago))
        seleccionar_tarjeteCredito.click()

    def set_adicionar_tarjeta_credito(self):
        WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located((By.CSS_SELECTOR, '.pp-row.disabled'))).click()
    ...
    def set_completar_codigo_tarjeta_Credito(self, numCodigo):
        completarCodigo = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.completar_campo_codigo))
    ...
    def get_obtener_tarjeta_credito(self):
        agregarTarjeta = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.completar_campo_tarjeta))
    ...
    def set_clic_agregar_tarjeta_credito(self):
        clic_AgregarTajeta = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.btn_agregar_tarjeta))
    ...
    def set_clic_manta_y_panuelo(self):
        clicOpcion_MyP = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.opcion_manta_y_panuelo))
    ...
    def set_adicionar_helado(self):
        adicionar_helado = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.btn_adicionar_helado))
        adicionar_helado.click()
        adicionar_helado.click()

    def get_conseguir_total_helados(self):
        total_helado = WebDriverWait(self.driver, 10).until(
            e_c.presence_of_element_located(self.conseguir_total_helados))
...
class TestUrbanRoutes:
    ...
    def test_texto_costo_servicio_comfort(self):
        routes_page = UrbanRoutesPage(self.driver)
        routes_page.set_hacer_clic_btn_pedir_taxi()
        routes_page.set_clic_btn_comfort()
        assert 'Comfort\n$10' == routes_page.get_btn_comfort()

    def test_adicionar_num_telefono(self):
        routes_page = UrbanRoutesPage(self.driver)
        numTel = data.phone_number
        routes_page.set_ejecutar_agregar_num_telefono(numTel)
        numCodigo = retrieve_phone_code(self.driver)
        routes_page.set_codigo_num_telefono(numCodigo)
        assert routes_page.get_num_telefono() == numTel

    def test_adicionar_tarjeta_credito(self):
        routes_page = UrbanRoutesPage(self.driver)
        routes_page.set_clic_modo_pago_y_agregar_tarjeta()
        numeroTarjeta = data.card_number
        numCodigo = data.card_code
        routes_page.set_completar_campos_tarjeta_y_codigo(numeroTarjeta, numCodigo)
        assert routes_page.get_obtener_tarjeta_credito() == numeroTarjeta
        assert routes_page.get_obtener_codigo() == numCodigo
        routes_page.set_clic_salir_foco_agregar_tarjeta_cerrar_pop_up()
        assert routes_page.get_conseguir_modo_pago() == 'Tarjeta'

    def test_adicionar_mensaje_conductor(self):
        routes_page = UrbanRoutesPage(self.driver)
        mensajeConductor = data.message_for_driver
